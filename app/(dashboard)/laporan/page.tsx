...
import { fetchAllTransaksi, fetchDashboardStats } from "@/lib/services/pelanggan.service";
import type { Transaction } from "@/features/pelanggan/types";
import LaporanPageSkeleton from "./_components/LaporanPageSkeleton";
import LaporanTable from "./LaporanTable";

type Stats = {
  kategoriCount: { Green: number; Yellow: number; Red: number; Black: number };
};

type FilterType = "semua" | "belum_lunas" | "lunas";
...
export default function LaporanPage() {
  ...
  const filtered = data.filter((row) => {
    ...
    if (filter === "belum_lunas") {
      return row.status === "belum_lunas" || row.status === "cicilan";
    }
    if (filter === "lunas") return row.status === "lunas";
    return true;
  });

  const totalKasbon = data
    .filter((row) => row.type === "kasbon")
    .reduce((sum, row) => sum + row.nominal, 0);
  const totalBayar = data
    .filter((row) => row.type === "bayar")
    .reduce((sum, row) => sum + row.nominal, 0);
  const sisaPiutang = Math.max(totalKasbon - totalBayar, 0);
  const transaksiAktif = data.filter(
    (row) => row.type === "kasbon" && row.status !== "lunas",
  ).length;
  ...
  const totalSegmentedCustomers = stats
    ? Object.values(stats.kategoriCount).reduce((sum, count) => sum + count, 0)
    : 0;
  ...
  const kategoriList = [
    {
      ...
      note: "Tidak ada utang telat dan relasi pembayaran aman.",
    ...
    },
    {
      ...
      note: "Mulai perlu dipantau karena ada telat singkat.",
  ...
  return (
    <main className="min-h-screen -mx-3 -mt-3 px-3 pt-3 pb-4 lg:-mx-5 lg:-mt-4 lg:px-4 lg:pt-4 lg:pb-6">
          ...
          <div className="relative z-10 flex flex-wrap items-end justify-between gap-3 md:gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-white/85 md:text-base">
                Snapshot performa kasbon dan kualitas pelanggan bulan ini
              </p>
            </div>
      ...
      <section className="mt-4 rounded-3xl border border-orange-200 bg-white/95 p-4 backdrop-blur-sm dark:border-neutral-800 dark:bg-neutral-900/90 md:p-5">
            ...
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-xs font-bold transition border ${
                filter === item.key
                  ...
                  : "bg-white text-neutral-600 border-orange-200 hover:bg-orange-50 dark:bg-neutral-900 dark:text-neutral-200 dark:border-orange-800"
              }`}
            >
              {item.label}
            </button>
        ...
        <div className="mt-4">
          <LaporanTable key={filter} data={filtered} view={filter} />
        </div>
      )}
    </section>
    </main>
